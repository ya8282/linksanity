...
| `fix` | nothing to fix | proposals exist (dry run), or were applied (`--write`) | operational error (bad arguments, invalid config, a missing dependency — Playwright, when `js_domains` is set — or a failed write to `--output` or an unreadable domains/skip file), or `--write` refused a dirty tree | n/a — `fix` has no `--github-issue` |
...
| `2` | Operational error: bad arguments, an invalid config, a missing dependency (Playwright, when `js_domains` is set in the config), or a failed write to `--output` or an unreadable domains/skip file -- or `--write` refused a dirty working tree |

For `init`:

| Code | Meaning |
|---|---|
| `0` | All validation, config loading, and (unless `--no-measure`) the measuring scan passed. A real run then wrote the workflow (and baseline, if any); `--dry-run` printed them instead, without refusing when the target files already exist (it notes that a real run would refuse) |
| `2` | Bad arguments, non-TTY stdin without `--yes`, no path to scan, declining the competing-checker prompt, an invalid config, or a failed measuring scan -- all of these run before the dry-run check, so they exit `2` even under `--dry-run`. On a real (non-dry) run only: an existing workflow file (under `--yes`, or when you decline to overwrite it), an existing baseline file under `--yes`, or a failed write |

...
**The trap:** `check_images` is still parsed out of `linksanity.toml` regardless of which subcommand runs — `load_config` doesn't know or care that `crawl` has no matching flag. So `check_images = true` in your config file has no effect when you run `crawl`: no warning, no error, the field is silently read and then never acted on. See [Configuration](/guides/configuration) for this trap and the other fields that behave the same way.

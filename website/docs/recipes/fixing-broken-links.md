...
`--write` on a dirty tree exits `2`, the same code as any other operational error (bad arguments, invalid config, a missing dependency, a failed write to `--output`, or an unreadable domains/skip file) — check the exit code, not just whether files changed, when scripting this.
